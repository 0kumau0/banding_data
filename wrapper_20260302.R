...
dataset_test <- list()
# 【重要】coordsはデータフレームのまま、かつ列名x,yを持つようにする
...
# 【修正】行 = 調査回(total_surveys: 15), 列 = 個体(n_ind: 20) にします
...
dataset_test$detect <- detect_mat # 行列の向き修正済み
...
# wrapper関数 ---------------------------------------------------------------
# 1. ヘルパー関数: 指定IDのデータのみを持つsecradオブジェクトを作成する (修正版)
create_subset_secrad <- function(original_secrdata, ids, model_settings) {
  ...
  # 【重要】add_obsを使うことで nocc などの内部情報が正しく登録される
...
for(iter in 1:max_iter) {
  ...
  g <- numDeriv::grad(
    ...
    original_secrdata = secrdata_test, # secrdataオブジェクトを渡す
    model_settings = current_model_settings
  )
  
  # パラメータ更新 (シンプル版: モメンタムなし)
