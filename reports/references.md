...
### 日本語の教科書（2026-09-16 追加）

| | 確度 | |
|---|---|---|
| **瀧雅人『これならわかる深層学習入門』講談社** | ★★★ | 4章が勾配降下法。最急降下法 → SGD → モメンタム → AdaGrad → RMSprop → Adam の系譜。更新式は `Δw = −η m/√(v+ε)`（**`ε` がルートの内側**。原典 Kingma & Ba は外側）。バイアス補正 `1−βᵗ` あり |

**この本の式とこのリポジトリの実装の対応は `reports/20260916_adam_textbook.md` に全部書いた。**
違うのは符号（我々は最大化）・`η` がベクトル・`max_step` の clip・`ε` の位置の4点だけで、
**式そのものは同一**。破綻したのは式ではなく**既定値 `β₂ = 0.999` の前提（反復回数）**。
